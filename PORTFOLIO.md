...
| Split | Normal | Defect | 용도 |
|-------|--------|--------|------|
| Train | 5,610 | 0 | 정상 패턴 학습 |
| Validation | 1,296 | 1,296 | Threshold 최적화 |
| Test | 1,296 | 1,296 | 최종 평가 |
...
**4. 실험적 검증 결과**
| 모델 | Test Accuracy | 학습 시간 | 비고 |
|------|---------------|-----------|------|
| **ResNet18** | **99.2%** | ~30분 | ✅ 채택 |
| ResNet50 | 99.1% | ~60분 | 성능 차이 없음 |
| EfficientNet-B0 | 98.8% | ~45분 | 오히려 낮음 |

**5. 결론**
```
✅ 99.2% 정확도로 충분한 성능 달성
✅ 추가 복잡도는 성능 향상 없이 비용만 증가
...
#### 성능 비교
| 모델 | Test Accuracy | Macro F1 | ROC-AUC | 모델 크기 |
|------|---------------|----------|---------|-----------|
| **ResNet18 (기본)** | **99.2%** | **98.8%** | **0.998** | 46MB |
| ResNet18 + CBAM | 98.5% | 97.9% | 0.994 | 135MB |
...
Val: 정상 1,296장 + 결함 1,296장 (Threshold 계산용)
Test: 정상 1,296장 + 결함 1,296장
...
# 결과:
# - Train: 5,610 정상 이미지
# - Val: 1,296 정상 + 1,296 불량
# - Test: 1,296 정상 + 1,296 불량
...
### 1. 높은 검출 정확도
- CT CNN: **99.2% Accuracy**
- RGB AE: **98.85% Accuracy**, ROC-AUC **0.9999** (Validation)
