"""RGB 데이터 배터리 ID별 Split 재생성

문제: 구버전 split(rgb_train/val/test.txt)은 같은 배터리의 다른 이미지가
      train/val/test에 분산되어 데이터 누수 발생
해결: 배터리 ID 단위로 split하여 같은 배터리는 같은 split에만 존재하도록 수정

파일명 예: RGB_cell_cylindrical_0136_067.png → 배터리 ID: cell_cylindrical_0136
"""
...
# 경로 설정
splits_dir = Path('training/data/splits/rgb')


def extract_battery_id(filepath: str) -> str:
    """파일 경로에서 배터리 ID 추출

    예: RGB_cell_cylindrical_0136_067.png → cell_cylindrical_0136
    """
    filename = Path(filepath).stem  # RGB_cell_cylindrical_0136_067
    # RGB_ 제거 후 마지막 필드(이미지 번호) 제외
    parts = filename.replace('RGB_', '').split('_')
    if len(parts) >= 3:
        # 마지막 필드 제외하고 배터리 ID 구성
...
# 1. 모든 데이터 로드
print("=" * 60)
print("RGB 데이터 배터리 단위 Split 재생성")
print("=" * 60)

all_data = []
for split_file in ['rgb_train.txt', 'rgb_val.txt', 'rgb_test.txt']:
...
for battery_id, files in battery_data.items():
    ...
    if len(labels) > 1:
        print(f"  주의: {battery_id}: 여러 클래스 {labels}")
...
# 8. 저장 (기존 rgb_ 파일은 건드리지 않고 새 파일로 저장)
for name, files in [('rgb_battery_train.txt', train_files),
                    ('rgb_battery_val.txt', val_files),
                    ('rgb_battery_test.txt', test_files)]:
...
# 10. 구버전 split 배터리 ID 겹침 확인 (참고용)
print(f"\n[8] 구버전 split 배터리 ID 겹침 확인 (참고)...")
old_splits = {}
for split_name in ['rgb_train.txt', 'rgb_val.txt', 'rgb_test.txt']:
        ...
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if line:
                    parts = line.split('\t')
                    if len(parts) >= 2:
                        bat_id = extract_battery_id(parts[0])
                        if bat_id:
                            bat_ids.add(bat_id)
        old_splits[split_name] = bat_ids

if len(old_splits) == 3:
    old_train = old_splits['rgb_train.txt']
    old_val = old_splits['rgb_val.txt']
    old_test = old_splits['rgb_test.txt']
    print(f"  구버전 Train-Val 겹침: {len(old_train & old_val)}개")
    print(f"  구버전 Train-Test 겹침: {len(old_train & old_test)}개")
    print(f"  구버전 Val-Test 겹침: {len(old_val & old_test)}개")

print("\n" + "=" * 60)
print("RGB Split 재생성 완료!")
print("=" * 60)
